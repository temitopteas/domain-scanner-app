...
/**
 * Scores a full list and returns only survivors above threshold.
 * @param {string[]} domains
 * @param {*} profile
 * @param {number} threshold  default 6.0 — tune based on how many survivors you want
 * @param {boolean} preserveOrder  if true, keeps the original upload order
 *   instead of sorting by score. Needed when pattern filtering is being
 *   bypassed entirely — sorting by a score you've deliberately ignored
 *   would still silently push domains like "facebook.com" to the back of
 *   the line before the per-request grounding cap ever reaches them.
 */
function scoreAndFilter(domains, profile, threshold = 6.0, preserveOrder = false) {
  const scored = domains.map((d) => scoreDomain(d, profile));
  const survivors = scored.filter((s) => s.score >= threshold);
  return preserveOrder ? survivors : survivors.sort((a, b) => b.score - a.score);
}
