...
const DOMAIN_BATCH_SIZE = 500; // domains sent to /api/scan-domains per request (stage-1 filtering is cheap; only survivors get live-checked, capped server-side)
const EMAIL_BATCH_SIZE = 10; // emails sent to /api/verify-emails per request, checked concurrently — matches the server's concurrency cap
const GEO_BATCH_SIZE = 5000; // domains sent to /api/scan-geo-domains per request — fully local computation now (no live checks), so this can be large
...
function DomainScanner() {
  ...
  const [skipPatternFilter, setSkipPatternFilter] = useState(false);
  ...
  function handleDomainsFile(e) {
    ...
    Papa.parse(file, {
      header: false,
      complete: (res) => {
        // Filters out an accidental header row ("domain", "url", etc.) and
        // anything that clearly isn't domain-shaped (no dot in it).
        const HEADER_WORDS = new Set(["domain", "domains", "url", "website", "expired domain", "candidate"]);
        const cleaned = res.data
          .flat()
          .filter(Boolean)
          .map((d) => d.toString().trim())
          .filter((d) => d.includes(".") && !HEADER_WORDS.has(d.toLowerCase()));
        setDomains(cleaned);
      },
    });
  }
  ...
  async function run() {
    ...
    const effectiveThreshold = skipPatternFilter ? -100 : threshold;
    ...
    for (const batch of batches) {
      try {
        const res = await fetch("/api/scan-domains", {
          ...
          body: JSON.stringify({ domains: batch, salesData, threshold: effectiveThreshold, skipPatternFilter }),
        });
  ...
  return (
    <div>
      ...
      <label style={{ display: "block", marginBottom: 8 }}>
        <input
          type="checkbox"
          checked={skipPatternFilter}
          onChange={(e) => setSkipPatternFilter(e.target.checked)}
        />{" "}
        Skip pattern filtering — check extensions for every domain in the
        list, regardless of naming pattern. Useful for testing with known
        domains (e.g. facebook.com) to confirm the extension check itself
        is working, or for small lists where you want every domain checked.
      </label>
      <input
        type="number"
        step="0.5"
        value={threshold}
        disabled={skipPatternFilter}
      ...
      {lastNote && !error && (
        <p style={{ fontSize: 12, color: "#a35a00", background: "#fff8ea", padding: 8 }}>
          {lastNote}
        </p>
      )}
  ...
  return (
    <div>
      ...
      <p style={{ fontSize: 12, color: "#666", marginTop: 12 }}>
        Every domain shown here contains a real US city OR state — checked
        against a bundled list of 20,000+ cities and all 50 states (full
        names and abbreviations, e.g. "Texas" or "TX"). A short,
        coincidental-looking city match (e.g. "Alto" inside "reALTOr")
        automatically defers to a real state name in the same domain when
        one is present. Root keyword sales stats are an EXACT match only —
        a domain root of "HandymanExperts" shows 0 sales if your sheet only
        has "Handyman".
      </p>
